@echo off
REM
REM   test/runtests.bat
REM   Run automated tests script for Microsoft Windows XP or (possibly) newer.
REM
...
:COMPILE_ERROR
ECHO #
ECHO # ERROR: Tests compile error.
ECHO #
IF NOT EXIST RESULTS.ERRORS (
	ECHO Tests compile error >> RESULTS.ERRORS
)
IF EXIST RESULTS.PASSED DEL RESULTS.PASSED > NUL
GOTO END_SCRIPT
