...
METHOD resolveColumnIndex( ncColumn )
   ...
   IF ( niColumnIndex < 1 ) .OR. ( niColumnIndex > niTotalColumns )
      ...
      oError:operation := ALLTRIM( HB_CStr( ncColumn ) )
      Throw( oError )
   ENDIF
...
METHOD getString( ncColumn )
   ...
   //IF !( niType $ { SQLITE_TEXT, SQLITE3_TEXT } )
   IF ( HB_ASCAN( { SQLITE_TEXT, SQLITE3_TEXT }, niType ) == 0 )
 ...
 *  abstracted by this façade using a <code>TEXT</code> data type column with
...
METHOD getDate( ncColumn )
   ...
   IF !( aMatches == NIL )
      IF ( LEN( aMatches ) == 4 )
         dValue := HB_STOD( STRZERO( VAL( aMatches[ 2 ] ), 4, 0 ) + ;
 ...
 *  own. Boolean values are abstracted by this façade using a <code>TEXT</code>
...
METHOD getBoolean( ncColumn )
   LOCAL cBruteValue := ::getString( ncColumn )
   LOCAL nMatches := HB_ASCAN( { "yes", "ok", "true", ".t." }, ;
